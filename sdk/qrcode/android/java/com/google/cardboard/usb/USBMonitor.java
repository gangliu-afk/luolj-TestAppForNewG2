...
package com.google.cardboard.usb;
...
import com.google.cardboard.utils.HandlerThreadHandler;
...
public final class USBMonitor {
	...
	private static final String MY_TAG = "PHONEAR";

	private static final String ACTION_USB_PERMISSION_BASE = "com.google.cardboard.USB_PERMISSION";
...
//			if (DEBUG) Log.i(TAG, "unregister:");
	...
	public synchronized boolean requestPermission(final UsbDevice device) {
		if (DEBUG) Log.v(MY_TAG, "requestPermission:device=" + device.getVendorId() + ":" + device.getProductId());
			...
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					...
					try {
						...
						if (DEBUG) Log.v(MY_TAG, "usbManager get permission");
					} catch (final Exception e) {
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (ACTION_USB_PERMISSION.equals(action)) {
				if(DEBUG) Log.d(MY_TAG, "ACTION_USB_PERMISSION");
	...
	private final Runnable mDeviceCheckRunnable = new Runnable() {
		...
		public void run() {
			...
			if (DEBUG) Log.d(MY_TAG, "device size" + n);
			...
			synchronized (mHasPermissions) {
				...
				if (DEBUG) Log.d(MY_TAG, "permission size" + m);
			}
			if ((n > mDeviceCounts) || (m > hasPermissionCounts)) {
				...
				if (mOnDeviceConnectListener != null) {
					for (int i = 0; i < n; i++) {
						...
						if (DEBUG) Log.d(MY_TAG, "device id" + i);
	...
	public static UsbDeviceInfo updateDeviceInfo(final UsbManager manager, final UsbDevice device, final UsbDeviceInfo _info) {
		...
		if (device != null) {
			...
			if ((manager != null) && manager.hasPermission(device)) {
				...
				if(DEBUG) Log.d(MY_TAG, "updateDeviceInfo");
